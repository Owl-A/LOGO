...
public:
  Cursor(float loX, float loY, float hiX, float hiY, float step)
    ...
    int windowBound[] = {0, 1, 1, 2, 3, 4, 4, 5};

    float turtle[] = {0.00, 0.05, -0.05, -0.05, 0.05, -0.05};

    float line[] = {0.0, 0.0, 0.0, step};
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(windowBound), windowBound,
                 GL_STATIC_DRAW);
  ...
  std::queue<AST_Atomic *> *get_queue() { return &input_queue; }
  ...
  void render_lines() {
    ...
    for (int i = 0; i < buffer_x.size(); i++) {
      ...
      model = glm::rotate(model, glm::radians(buffer_ang[i]),
  ...
  void _forward(float value) {
    ...
    if (t >= value) {
      ...
      if (axis == 1) { // X hit extreme
        if (std::abs(x - loX) < EPSILON)
          x = hiX;
        else
          x = loX;
      } else { // Y hit extreme
        if (std::abs(y - loY) < EPSILON)
          y = hiY;
        else
          y = loY;
      }
