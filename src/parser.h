...
class Parser {
  ...
  AST *block() {
    std::vector<AST *> ret;
    while (current->type != RIGHT_BRACKET)
  ...
  AST *statement() {
    if (current->type == FORWARD || current->type == BACKWARD ||
        current->type == RIGHT || current->type == LEFT) {
...
public:
  Parser(std::string buf) : lexer(buf) { current = lexer.next(); }
